...
| Proto compiler      | `buf` v1.50+ + `protoc-gen-go` v1.36 + `protoc-gen-go-grpc` v1.6                          |
...
├── buf.yaml                    # buf module config (deps: buf.build/googleapis/googleapis)
├── buf.gen.yaml                # buf code generation config (plugins + output dirs)
├── buf.lock                    # buf dependency lock file (generated by buf dep update)
├── gen/
...
resolved automatically via `buf.build/googleapis/googleapis` declared in `buf.yaml`.
No vendored files are needed — `buf dep update` downloads and locks the dependency.

`mage gen` runs `buf generate` (configured by `buf.gen.yaml`) to produce all Go bindings.
...
- Do not delete `buf.yaml`, `buf.gen.yaml`, or `buf.lock` — they are required by `mage gen`
